...
// console.log('gameLavel: '+gameLavel);
...
function MiniMax(tablica_gry, znak_gracza, ilosc_zajetych_pol, deep, alfa, beta, obj){
var
   m, mmx, fi, fj;
          ...
          if (Wyg(tablica_gry,O) === true){ return -1; }
              ...
              if ((ilosc_zajetych_pol === (nmax*nmax)) || (deep === 4 && nmax > 3) || (deep === 1 && gameLevel === 'possible')){ return 0; }
                ...
                if (obj.zatrzymanie === false){
                    ...
                    if (deep === 0) obj.zatrzymanie = false;

                    if (beta <= alfa) break;
                    
					if ((obj.zatrzymanie === true) && (deep > 0)) break;
					  }
                    if (beta <= alfa) break;
     ...
     for(fi = 0; fi < nmax; ++fi){
       for(fj = 0; fj < nmax; ++fj)
        if(tab_gry[fi][fj] === null){
		  tab_gry[fi][fj] = X;
		  m = MiniMax(tab_gry,X,liczba_pol+1,0,-10,10,obj1);
		  tab_gry[fi][fj] = null;
          if (mmx <= m){
